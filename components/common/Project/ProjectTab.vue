<template>
  <div class="border-b flex items-center gap-6">
    <Button v-for="item, index in tabs" class="text-muted-foreground pl-0 gap-1 relative"
      :class="`${isActive === item.label && 'text-primary'}`" variant="none" @click="item.action">
...
import { Settings, Group } from 'lucide-vue-next';
//state
